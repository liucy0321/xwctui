...
export const useWatermark = (props: IWatermarkProps) => {
  const { height, width, route, color, fontSize, transparent, name } = props;
  let H = height || 150; //  水印行高
  let W = width || 200; // => 水印宽度
  let R = route || -32; //  => 旋转度数（可为负值）
  let C = color || "#d3cfcf"; // => 水印字体颜色
  let S = fontSize || 13; //  => 水印字体的大小
  let O = transparent || 0.5; // => 水印透明度（0~1之间取值）
  ...
  for (let i = 0; i < TpLine; i++) {
    StrLine += `<span style="display: inline-block; line-height:${H}px; width:${W}px; text-align: center; transform:rotate(${R}deg); color:${C}; font-size:${S}px; opacity:${O};">${name}</span>`;
  }
...
export const dumpWaterMark = () => {
  if (document.getElementById("tp-watermark") != null) {
    document.getElementById("tp-watermark")?.remove();
  }
};
